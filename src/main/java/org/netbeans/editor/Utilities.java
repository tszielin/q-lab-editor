...
/**
 * Various useful editor functions. Some of the methods have the same names and signatures like in
 * javax.swing.Utilities but there is also many other useful methods. All the methods are static so
 * there's no reason to instantiate Utilities. All the methods working with the document rely on
 * that it is locked against modification so they don't acquire document read/write lock by
 * themselves to guarantee the full thread safety of the execution. It's the user's task to lock the
 * document appropriately before using methods described here. Most of the methods require
 * org.netbeans.editor.BaseDocument instance not just the javax.swing.text.Document. The reason for
 * that is to mark that the methods work on BaseDocument instances only, not on generic documents.
 * To convert the Document to BaseDocument the simple conversion (BaseDocument)target.getDocument()
 * can be done or the method getDocument(target) can be called. There are also other conversion
 * methods like getEditorUI(), getKit() or getKitClass().
 * 
...
public class Utilities {
  ...
  /**
   * Get the starting position of the row while providing relative count of row how the given
   * position should be shifted. This is the most efficient way how to move by lines in the document
   * based on some position. There is no similair getRowEnd() method that would have shifting
   * parameter.
   * 
   * @param doc
   *          document to operate on
   ...
   *          position in document where to start searching
   * @param lineShift
   *          shift the given offset forward/back relatively by some amount of lines
   * @return position of the start of the row or -1 for invalid position
   */
  ...
  /**
   * Get the first non-white character on the line. The document.isWhitespace() is used to test
   * whether the particular character is white space or not.
   * 
   * @param doc
   *          document to operate on
   * @param offset
   *          position in document anywhere on the line
   * @return position of the first non-white char on the line or -1 if there's no non-white
  ...
  public static int getRowFirstNonWhite(BaseDocument doc, int offset) throws BadLocationException {
    ...
    return getFirstNonWhiteFwd(doc, lineElement.getStartOffset(), lineElement.getEndOffset() - 1);
  }

  /**
   * Get the last non-white character on the line. The document.isWhitespace() is used to test
   * whether the particular character is white space or not.
   * 
   * @param doc
   *          document to operate on
   * @param offset
   *          position in document anywhere on the line
   * @return position of the last non-white char on the line or -1 if there's no non-white character
   *         on that line.
   */
  public static int getRowLastNonWhite(BaseDocument doc, int offset) throws BadLocationException {
    ...
    Element lineElement = doc.getParagraphElement(offset);
    return getFirstNonWhiteBwd(doc, lineElement.getEndOffset() - 1, lineElement.getStartOffset());
  }
  ...
  /**
   * Get indentation on the current line. If this line is white then go either up or down an return
   * indentation of the first non-white row. The <tt>getRowFirstNonWhite()</tt> is used to find the
   * indentation on particular line.
   * 
   * @param doc
   *          document to operate on
   * @param offset
   *          position in document anywhere on the line
   * @param downDir
   *          if this flag is set to true then if the row is white then the indentation of the next
   *          first non-white row is returned. If it's false then the indentation of the previous
   *          first non-white row is returned.
   ...
   * @return position of the character that is at the one line above at the required x-coordinate
   *         value
   */
...
//    BaseDocument doc = (BaseDocument)c.getDocument();
  ...
  /**
   * Get start of the current word. If there are no more words till the begining of the document,
...
//    int docLen = doc.getLength();
   ...
   *          position in document where to start searching
   * @param limitPos
   *          position in document (greater or equal than offset) where the search will stop
   *          reporting unsuccessful search by returning -1
  ...
  /**
   * Get first white character in document in backward direction. The character right before the
   * character at position offset will be searched as first.
   * 
   * @param doc
   *          document to operate on
   * @param offset
   ...
   *          position in document where to start searching
   * @param limitPos
   *          position in document (lower or equal than offset) where the search will stop reporting
   *          unsuccessful search by returning -1
   * @return position of the first white character or -1
   */
  ...
  /**
   * Get first non-white character in document in backward direction. The character right before the
   * character at position offset will be searched as first.
   * 
   * @param doc
   *          document to operate on
  ...
  /**
   * Get the identifier around the given position or null if there's no identifier around the given
   * position. The identifier is not verified against SyntaxSupport.isIdentifier().
   * 
   * @param c
   *          JTextComponent to work on
   * @param offset
   *          position in document - usually the caret.getDot()
   * @return the block (starting and ending position) enclosing the identifier or null if no
  ...
  public static int[] getIdentifierBlock(JTextComponent c, int offset) throws BadLocationException {
    ...
    if (idStart >= 0) {
      ...
      if (idEnd >= 0) {
        ...
        String trim = id.trim();
        if (trim.length() == 0 ||
            (trim.length() == 1 && !Character.isJavaIdentifierPart(trim.charAt(0)))) {
          ...
          if ((id != null) && (id.length() != 0) && (id.indexOf(".") != -1)) { // NOI18N
  ...
  /**
   * Get the identifier around the given position or null if there's no identifier around the given
   * position. The identifier must be accepted by SyntaxSupport.isIdnetifier() otherwise null is
   * returned.
   * 
   * @param doc
   *          document to work on
  ...
  /**
   * Get the selection if there's any or get the identifier around the position if there's no
   * selection.
   * 
   * @param c
   *          component to work with
  ...
  /**
   * Get the identifier before the given position (ending at given offset) or null if there's no
   * identifier
   */
      ...
      switch (type) {
        ...
        case CASE_SWITCH:
          if (Character.isUpperCase(orig[i])) {
            ...
            if (Character.isLowerCase(orig[i])) {
              changed[i] = Character.toUpperCase(orig[i]);
            }
          break;
      }
  ...
  public static boolean isRowWhite(BaseDocument doc, int offset) throws BadLocationException {
    Element lineElement = doc.getParagraphElement(offset);
    offset = doc.find(new FinderFactory.NonWhiteFwdFinder(doc), lineElement.getStartOffset(),
  ...
  public static void setStatusBarText(JTextComponent c, String cellName, String text) {
    setStatusBarText(c, cellName, text, false);
  }
  
  public static void setStatusBarText(JTextComponent c, String cellName, String text, boolean resize) {
    setStatusBarText(c, cellName, text, resize, null);
  }
  
  public static void setStatusBarText(JTextComponent c, String cellName, String text,
      boolean resize, Color color) {
    if (getEditorUI(c) != null) {
      StatusBar sb = getEditorUI(c).getStatusBar();
      if (sb != null) {
        JLabel label = sb.getCellByName(cellName);
        if (label != null) {
          label.setVisible(text != null && text.trim().length() > 0);
          sb.setText(cellName, text, resize);
          if (label.isVisible()) {
  ...
  public static void setStatusBarText(JTextComponent c, String cellName, String text, Coloring extraColoring) {
    StatusBar sb = getEditorUI(c).getStatusBar();
    if (sb != null && sb.getCellByName(cellName) != null) {
//      sb.setText(cellName, text, extraColoring);
...
//      sb.setText(StatusBar.CELL_MAIN, text, extraColoring);
  ...
  public static String debugPosition(BaseDocument doc, int offset) {
    ...
    if (offset >= 0) {
      try {
        ...
        ret = LocaleSupport.getString(WRONG_POSITION_LOCALE) + ' ' + offset + " > " +
            doc.getLength(); // NOI18N
      }
    }
    else {
  ...
  /**
   * Display the identity of the document together with the title property and stream-description
   * property.
   */
  public static String debugDocument(Document doc) {
    return "<" + System.identityHashCode(doc) + ", title='" +
        doc.getProperty(Document.TitleProperty) + "', stream='" +
  ...
  /**
   * Helper method to obtain instance of EditorUI (extended UI) from the existing JTextComponent. It
   * doesn't require any document locking.
   * 
   * @param target
   *          JTextComponent for which the extended UI should be obtained
   * @return extended ui instance or null if the component.getUI() does not return BaseTextUI
   *         instance.
   */
  ...
  /**
   * Helper method to obtain instance of editor kit from existing JTextComponent. If the kit of the
   * component is not an instance of the <tt>org.netbeans.editor.BaseKit</tt> the method returns
   * null. The method doesn't require any document locking.
   * 
   * @param target
  ...
  /**
   * Helper method to obtain editor kit class from existing JTextComponent. This method is useful
   * for example when dealing with Settings. The method doesn't require any document locking.
   * 
   * @param target
   *          JTextComponent for which the editor kit should be obtained
  ...
  /**
   * Helper method to obtain instance of BaseDocument from JTextComponent. If the document of the
   * component is not an instance of the <tt>org.netbeans.editor.BaseDocument</tt> the method
   * returns null. The method doesn't require any document locking.
   * 
   * @param target
  ...
  /**
   * Get the syntax-support class that belongs to the document of the given component. Besides using
   * directly this method, the <tt>SyntaxSupport</tt> can be obtained by calling
   * <tt>doc.getSyntaxSupport()</tt>. The method can return null in case the document is not an
   * instance of the BaseDocument. The method doesn't require any document locking.
   * 
   * @param target
  ...
  /**
   * Creates nice textual description of sequence of KeyStrokes. Usable for displaying
   * MultiKeyBindings. The keyStrokes are delimited by space.
   * 
   * @param Array
  ...
  /**
   * Creates nice textual representation of KeyStroke. Modifiers and an actual key label are
   * concated by plus signs
   * 
   * @param the
  ...
  /**
   * @return slight modification of what KeyEvent.getKeyText() returns. The numpad Left, Right, Down,
   *         Up get extra result.
   */
  ...
  private static void checkOffsetValid(Document doc, int offset) throws BadLocationException {
    ...
    if (offset < 0 || offset > docLen) {
      throw new BadLocationException("Invalid offset, document length=" + docLen, offset);
    }
  }
