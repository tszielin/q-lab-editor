...
public class BaseKit extends DefaultEditorKit {
  ...
  static SettingsChangeListener settingsListener = new SettingsChangeListener() {
    public void settingsChange(SettingsChangeEvent evt) {
      ...
      boolean clearActions = (settingName == null ||
          SettingsNames.CUSTOM_ACTION_LIST.equals(settingName) || SettingsNames.MACRO_MAP.equals(settingName));
      ...
      if (clearActions) {
        ...
        /*
         * Iterator i = kitActions.entrySet().iterator(); while (i.hasNext()) { Map.Entry me =
         * (Map.Entry) i.next(); updateActionSettings((Action[]) me.getValue(), evt, (Class)
         * me.getKey()); }
         */}
    }
  };
   ...
   * 
   * @param doc
   *          document to operate on. It can be null in the cases the syntax creation is not related
  ...
  /**
   * This methods gives the class of the first component in the component hierarchy that should be
   * called when requesting focus for the other component.
   * 
   * @return class of the component that will be searched in the parents in the hierarchy or null to
   *         do no search.
   */
  ...
  /**
   * Create actions that this kit supports. To use the actions of the parent kit it's better instead
   * of using super.createActions() to use getKit(super.getClass()).getActions() because it can
   * reuse existing parent actions.
   */
  protected Action[] createActions() {
    return new Action[]{new DefaultKeyTypedAction(), new InsertContentAction(),
        new InsertBreakAction(), new InsertTabAction(),
        new DeleteCharAction(deletePrevCharAction, false),
        new DeleteCharAction(deleteNextCharAction, true), new ReadOnlyAction(),
        new WritableAction(), cutActionDef, copyActionDef, pasteActionDef, new BeepAction(),
        new UpAction(upAction, false), new UpAction(selectionUpAction, true),
        new PageUpAction(pageUpAction, false), new PageUpAction(selectionPageUpAction, true),
        new DownAction(downAction, false), new DownAction(selectionDownAction, true),
        ...
        new EndWordAction(endWordAction, false), new EndWordAction(selectionEndWordAction, true),
        new SelectWordAction(), new SelectLineAction(), new SelectAllAction(),
        ...
        new ActionFactory.JumpListNextComponentAction(), new ActionFactory.ScrollUpAction(),
        new ActionFactory.ScrollDownAction(), new ActionFactory.StartMacroRecordingAction(),
        new ActionFactory.StopMacroRecordingAction(), new ActionFactory.InsertDateTimeAction(),
        new ActionFactory.GenerateGutterPopupAction(), new ActionFactory.ToggleLineNumbersAction(),
  ...
  protected Action[] getMacroActions() {
    ...
    for (Iterator<String> it = macroMap.keySet().iterator(); it.hasNext();) {
      actions.add(new ActionFactory.RunMacroAction(it.next()));
    }
    return actions.toArray(new Action[0]);
  }

  /**
   * Get actions associated with this kit. createActions() is called to get basic list and then
   * customActions are added.
   */
  ...
  /**
   * Update the actions right after their creation was finished. The <code>getActions()</code> and
   * <code>getActionByName()</code> can be used safely in this method. The implementation must call
   * <code>super.updateActions()</code> so that the updating in parent is performed too.
   */
  ...
  public static class DefaultKeyTypedAction extends BaseAction {
    ...
    /**
     * Hook to insert the given string at the given position into the given document in insert-mode,
     * no selection, writeable document. Designed to be overridden by subclasses that want to
     * intercept inserted characters.
     */
    ...
    /**
     * Hook to insert the given string at the given position into the given document in insert-mode
     * with selection visible Designed to be overridden by subclasses that want to intercept
     * inserted characters.
     */
    protected void replaceSelection(JTextComponent target, int dotPos, Caret caret, String str,
        boolean overwrite) throws BadLocationException {
      target.replaceSelection(str);
    }

    /**
     * Check whether there was any important character typed so that the line should be possibly
     * reformatted.
     */
    ...
    public void actionPerformed(ActionEvent evt, JTextComponent target) {
      if (target != null) {
        ...
        if (caret.isSelectionVisible()) { // block selected
          try {
            doc.getFormatter().changeBlockIndent(doc, target.getSelectionStart(),
                target.getSelectionEnd(), +1);
          }
  ...
  /**
   * Compound action that gets and executes its actions depending on the kit of the component. The
   * other advantage is that it doesn't create additional instances of compound actions.
   */
      ...
      if (target != null) {
        ...
        try {
          ...
          if (!select && caret.isSelectionVisible()) {
            ...
            pos = caret.getDot();
          int dot = target.getUI().getNextVisualPositionFrom(target, pos, null,
...
//          BaseDocument doc = (BaseDocument)target.getDocument();
  ...
  public static class BeginLineAction extends BaseAction {
    ...
    /**
     * Whether the action should go to the begining of the text on the line or to the first column
     * on the line
     */
  ...
  public static class BeginAction extends BaseAction {
    ...
    public BeginAction(String nm, boolean select) {
      super(nm, MAGIC_POSITION_RESET | ABBREV_RESET | UNDO_MERGE_RESET | WORD_MATCH_RESET |
          SAVE_POSITION | CLEAR_STATUS_TEXT);
      this.select = select;
    }
