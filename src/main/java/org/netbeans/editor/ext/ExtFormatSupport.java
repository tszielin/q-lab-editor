...
/**
 * Extended format-support offers comment-token support, token-and-text operations and other
 * support.
 * 
...
public class ExtFormatSupport extends FormatSupport {
   ...
   * 
   * @param fromPosition
   *          the position from which to start counting. If it's EOL, it's counted.
   * @param toPosition
   *          the ending position. If it points at EOL, it's ignored from the total count. It is
   *          necessary for the second position to follow the first one.
   */
  ...
  /**
   * Is the given token a comment token? By default it returns false but it can be redefined in
   * descendants.
   */
   ...
   * 
   * @param startPosition
   *          position from which the search starts. For the backward search the character right at
   *          startPosition is not considered as part of the search.
   * @param limitPosition
   *          position where the search will be broken reporting that nothing was found. It can be
   *          null to search till the end or begining of the chain (depending on direction).
   * @param stopOnEOL
   *          whether stop and return EOL token or continue search if EOL token is found.
   * @param backward
   *          whether search in backward direction.
   * @return first non-whitespace token or EOL or null if all the tokens till the begining of the
   *         chain are whitespaces.
   */
  ...
  /**
   * Get the start of the area of line where there is only whitespace or comment till the end of the
   * line.
   * 
   * @param pos
   ...
   */
  public TokenItem insertImageToken(TokenItem beforeToken, ImageTokenID tokenID,
   ...
   * 
   * @param startToken
   *          token from which to start searching. For backward search this token is excluded from
   *          the search.
   * @param limitToken
   *          the token where the search will be broken reporting that nothing was found. It can be
   *          null to search till the end or begining of the chain (depending on direction). For
   *          forward search this token is not considered to be part of search, but for backward
   *          search it is.
   * @param tokenID
   *          token-id to be searched. If null the token-id of the tokens inspected will be ignored.
   * @param tokenImage
   *          text of the token to find. If null the text of the tokens inspected will be ignored.
   * @param backward
   *          true for searching in backward direction or false to serach in forward direction.
  ...
  /**
   * Find the first non-whitespace and non-comment token in the given direction. This is similair to
   * <tt>findImportant()</tt> but it operates over the tokens.
   * 
   * @param startToken
   *          token from which to start searching. For backward search this token is excluded from
   *          the search.
  ...
  /**
   * This method can be used to find a matching brace token. Both the token-id and token-text are
   * used for comparison of the starting token.
   * 
   * @param startToken
   *          token from which to start. It cannot be null. For backward search this token is
   *          ignored and the previous one is used.
   * @param limitToken
   *          the token where the search will be broken reporting that nothing was found. It can be
   *          null to search till the end or begining of the chain (depending on direction). For
   *          forward search this token is not considered to be part of search, but for backward
  ...
  public TokenItem findMatchingToken(TokenItem startToken, TokenItem limitToken,
    ...
    while (token != null && token != limitToken) {
      if (tokenEquals(token, matchTokenID, startTokenContextPath, matchTokenImage)) {
        ...
        if (tokenEquals(token, startTokenID, startTokenContextPath, startText)) {
          depth++;
        }
  ...
  public TokenItem findMatchingToken(TokenItem startToken, TokenItem limitToken,
    ...
    return findMatchingToken(startToken, limitToken, matchTokenID, matchTokenID.getImage(),
        backward);
  }

  /**
   * Search for any of the image tokens from the given array and return if the token matches any
   * item from the array. The index of the item from the array that matched can be found by calling
   * <tt>getIndex()</tt> method. It is suitable mainly for the image-token-ids.
   * 
   * @param startToken
   *          token from which to start. For backward search this token is excluded from the search.
   * @param limitToken
   *          the token where the search will be broken reporting that nothing was found. It can be
   *          null to search till the end or begining of the chain (depending on direction). For
   *          forward search this token is not considered to be part of search, but for backward
  ...
  /**
   * Get the index of the token in the given token-id-and-text array or -1 if the token is not in
   * the array.
   */
  ...
  /**
   * Get the character at the given position. The caller must care about not to pass the
   * end-of-chain position to this method.
   */
