...
/**
 * Unlike the formatter class, the ExtFormatter concentrates on providing a support for the real
 * formatting process. Each formatter (there's only one per each kit) can contain one or more
 * formatting layers. The <tt>FormatLayer</tt> operates over the chain of the tokens provided by the
 * <tt>FormatWriter</tt>. The formatting consist of changing the chain of the tokens until it gets
 * the desired look. Each formatting requires a separate instance of <tt>FormatWriter</tt> but the
 * same set of format-layers is used for all the format-writers. Although the base implementation is
 * synchronized so that only one format-writer at time is processed by each format-writer, in
 * general it's not necessary. The basic implementation processes all the format-layers sequentialy
 * in the order they were added to the formatter but this can be redefined. The
 * <tt>getSettingValue</tt> enables to get the up-to-date value for the particular setting.
 * 
...
public class ExtFormatter extends Formatter implements FormatLayer {
  ...
  /**
   * Contains the names of the keys that were turned into custom settings and are no longer read
   * from the Settings.
   */
  ...
  /**
   * Return the name of this formatter. By default it's the name of the kit-class for which it's
   * created without the package name.
   */
  ...
  public void settingsChange(SettingsChangeEvent evt) {
    ...
    indentHotCharsAcceptor = SettingsUtil.getAcceptor(kitClass,
        ExtSettingsNames.INDENT_HOT_CHARS_ACCEPTOR, AcceptorFactory.FALSE);

    reindentWithTextBefore = SettingsUtil.getBoolean(kitClass,
        ExtSettingsNames.REINDENT_WITH_TEXT_BEFORE, false);
  }
  ...
  /**
   * This method allows to set a custom value to a setting thus overriding the value retrieved from
   * the <tt>Settings</tt>. Once done the value is no longer synchronized with the changes in
   * <tt>Settings</tt> for the particular setting. There's a map holding the names of all the custom
   * settings.
   */
  ...
  /**
   * Replace the format-layer with the layerName with the the given layer. If there's no such layer
   * with the same name, the layer is not replaced and false is returned.
   */
  ...
  /**
   * Whether do no formatting at all. If this method returns true, the FormatWriter will simply
   * write its input into the underlying writer.
   */
   ...
   * @param indentOnly
   *          whether just the indentation should be changed or regular formatting should be
   *          performed.
   * @return formatting writer. The text was already reformatted but the writer can contain useful
   *         information.
   */
   ...
   * 
   * @param target
   *          component to which the text was typed. Caaret position can be checked etc.
   * @param typedText
   *          text (usually just one character) that the user has typed.
   * @return block of the code to be reformatted or null if nothing should reformatted. It can
   *         return block containing just one character. The caller usually expands even one
   *         character to the whole line because less than the whole line usually doesn't provide
   *         enough possibilities for formatting.
  ...
  public int[] getReformatBlock(JTextComponent target, String typedText) {
    ...
    if (indentHotCharsAcceptor.accept(typedText.charAt(0))) {
      /*
       * This is bugfix 10771. See the issue for problem description. The behaviour before fix was
       * that whenever the lbrace is entered, the line is indented. This make no sense if a text
       * exist on the line before the lbrace. In this case we simply will not indent the line. This
       * is handled by the hasTextBefore check
       */
  ...
  /**
   * Whether the formatter accepts the given syntax that will be used for parsing the text passed to
   * the FormatWriter.
   * 
   * @param syntax
   *          syntax to be tested.
   * @return true whether this formatter is able to process the tokens created by the syntax or
   *         false otherwise.
   */
