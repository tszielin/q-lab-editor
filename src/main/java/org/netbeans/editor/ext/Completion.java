...
public class Completion implements PropertyChangeListener, SettingsChangeListener, ActionListener {
  ...
  /**
   * Last result retrieved for completion. It can become null if the document was modified so the
   * replacement position would be invalid.
   */
  ...
  /**
   * Component (usually scroll-pane) holding the view and the title and possibly other necessary
   * components.
   */
  ...
  public Completion(ExtEditorUI extEditorUI) {
    ...
    synchronized (extEditorUI.getComponentLock()) {
      ...
      if (component != null) {
        propertyChange(new PropertyChangeEvent(extEditorUI, ExtEditorUI.COMPONENT_PROPERTY, null,
            component));
      }
  ...
  /**
   * Reset the result of the last valid completion query. This is done for example after the
  ...
  /**
   * Return true when the pane exists and is visible. This is the preferred method of testing the
   * visibility of the pane instead of <tt>getPane().isVisible()</tt> that forces the creation of
   * the pane.
   */
  ...
  /**
   * Set the visibility of the view. This method should be used mainly for hiding the completion
   * pane. If used with visible set to true it calls the <tt>popup(false)</tt>.
   */
   ...
   * 
   * @param postRequest
   *          post the request instead of refreshing the view immediately. The
   *          <tt>ExtSettingsNames.COMPLETION_REFRESH_DELAY</tt> setting stores the number of
   *          milliseconds before the view is refreshed.
   */
  ...
  /**
   * Get the help and show it in the view. If the view is already visible perform the refresh of the
   * view.
   * 
   * @param postRequest
   *          post the request instead of displaying the view immediately. The
   *          <tt>ExtSettingsNames.COMPLETION_AUTO_POPUP_DELAY</tt> setting stores the number of
   *          milliseconds before the view is displayed. If the user presses a key until the delay
   *          expires nothing is shown. This guarantees that the user which knows what to write will
   *          not be annoyed with the unnecessary help.
   */
  ...
  /**
   * Called to do either displaying or refreshing of the view. This method can be called either
   * directly or because of the timer has fired.
   * 
   * @param evt
   *          event describing the timer firing or null if the method was called directly because of
   *          the synchronous showing/refreshing the view.
   */
      ...
      try {
        if ((caretPos != -1) &&
            (Utilities.getRowStart(component, component.getCaret().getDot()) != Utilities.getRowStart(
                component, caretPos)) && ((component.getCaret().getDot() - caretPos) > 0)) {
          getJDCPopupPanel().setCompletionVisible(false);
          caretPos = -1;
          return;
        }
          ...
          try {
            ...
            SwingUtilities.invokeLater(new Runnable() {
              public void run() {
                ...
                if (res != null) {
                  if (instantSubstitution && res.getData().size() == 1 && !isPaneVisible &&
                      instantSubstitutionImpl(caretPos)) {
                    setPaneVisible(false);
                    return;
                  }
                }
  ...
  /**
   * Performs instant text substitution, provided that result contains only one item and completion
   * has been invoked at the end of the word.
   * 
   * @param caretPos
   ...
   * This function is usually triggered upon pressing the Enter key.
   * 
  ...
  /**
   * Substitute the text with the longest common part of all the entries appearing in the view. This
   * function is usually triggered upon pressing the Tab key.
   * 
   * @return true if the substitution was performed false if not.
   */
  ...
  /**
   * Multistage task can test its cancel status after every atomic (non-cancellable) stage.
   */
