...
/**
 * Editor UI for the component. All the additional UI features like advanced scrolling, info about
 * fonts, abbreviations, keyword matching are based on this class.
 * 
...
//  private boolean noCompletionJavaDoc; // no completion available
  ...
  /*
   * public CompletionJavaDoc getCompletionJavaDoc() { if (completionJavaDoc == null) { if
   * (noCompletionJavaDoc) { return null; } synchronized (getComponentLock()) { JTextComponent
   * component = getComponent(); if (component != null) { BaseKit kit = Utilities.getKit(component);
   * if (kit != null && kit instanceof ExtKit) { completionJavaDoc =
   * ((ExtKit)kit).createCompletionJavaDoc(this); if (completionJavaDoc == null) {
   * noCompletionJavaDoc = true; } } } } } return completionJavaDoc; }
   */
