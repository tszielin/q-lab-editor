...
public class ExtKit extends BaseKit {
  private static final long serialVersionUID = 1028259743517913326L;

  /**
   * This action is searched and executed when the popup menu should be displayed to build the popup
   * menu.
   */
  ...
  /**
   * This action is searched and executed when the tool-tip should be displayed by tool-tip support
   * to build the tool-tip.
   */
  public static final String buildToolTipAction = "build-tool-tip"; // NOI18N

  /**
   * Open find dialog action - this action is defined in view package, but its name is defined here
   * for clarity
   */
  public static final String findAction = "find"; // NOI18N

  /**
   * Open replace dialog action - this action is defined in view package, but its name is defined
   * here for clarity
   */
  public static final String replaceAction = "replace"; // NOI18N

  /**
   * Open goto dialog action - this action is defined in view package, but its name is defined here
   * for clarity
   */
  ...
  public Completion createCompletion(ExtEditorUI extEditorUI) {
    return extEditorUI != null ? extEditorUI.getCompletion() : null;
  }

  /*
   * public CompletionJavaDoc createCompletionJavaDoc(ExtEditorUI extEditorUI) { return null; }
   */
  ...
  protected Action[] createActions() {
    Action[] extActions = new Action[]{new BuildPopupMenuAction(), new ShowPopupMenuAction(),
        new BuildToolTipAction(), new FindAction(), new ReplaceAction(), new GotoAction(),
        ...
        // new CodeSelectAction(), // the actionPerformed is empty and so I'm removing the action
        // from the list
        new EscapeAction(), new ExtDefaultKeyTypedAction(), new ExtInsertTabAction(),
        new ExtInsertBreakAction(), new CompletionShowAction(), new CompletionHideAction(),
        new JDCPopupPanelHideAction(), new JDCPopupPanelShowAction(), new ShiftInsertBreakAction()};
  ...
  public static class BuildPopupMenuAction extends BaseAction {
    ...
    /**
     * Add the action to the popup menu. This method is called for each action-name found in the
     * action-name-list. It should add the appopriate menu item to the popup menu.
     * 
     * @param target
     *          target component for which the menu is being constructed.
     * @param popupMenu
     *          popup menu to which this method should add the item corresponding to the
     *          action-name.
     * @param actionName
     *          name of the action to add. The real action can be retrieved from the kit by calling
        ...
        try {
          ...
          if (r != null) {
            if (target.getComponentPopupMenu() != null) {
              target.getComponentPopupMenu().show(target, r.x, r.y + r.height);
            }
            else {
  ...
  public static class BuildToolTipAction extends BaseAction {
    ...
    protected String buildText(JTextComponent target) {
      ...
      return (tts != null) ? target.getToolTipText(tts.getLastMouseEvent())
  ...
  public static class GotoAction extends BaseAction {
    ...
    /**
     * This method is called by the dialog support to translate the line offset to the document
     * position. This can be changed for example for the diff operations.
     * 
     * @param doc
     *          document to operate over
     * @param lineOffset
     *          the line offset to convert to position
     * @return document offset that corresponds to the row-start of the line with the line-number
     *         equal to (lineOffset + 1).
     */
  ...
  public static class GotoDeclarationAction extends BaseAction {
    ...
    public GotoDeclarationAction() {
      super(gotoDeclarationAction, ABBREV_RESET | MAGIC_POSITION_RESET | UNDO_MERGE_RESET |
          SAVE_POSITION);
    }
  ...
  public static class ToggleCaseIdentifierBeginAction extends BaseAction {
    ...
    ToggleCaseIdentifierBeginAction() {
      super(toggleCaseIdentifierBeginAction, ABBREV_RESET | MAGIC_POSITION_RESET |
          UNDO_MERGE_RESET | WORD_MATCH_RESET);
    }
  ...
  public static class CodeSelectAction extends BaseAction {
    ...
    public void actionPerformed(ActionEvent evt, JTextComponent target) {
      /*
       * if (target != null) { BaseDocument doc = (BaseDocument)target.getDocument(); SyntaxSupport
       * sup = doc.getSyntaxSupport(); Caret caret = target.getCaret(); try { int bracketPos =
       * sup.findUnmatchedBracket(caret.getDot(), sup.getRightBrackets()); if (bracketPos >= 0) {
       * caret.setDot(bracketPos); while (true) { int bolPos = Utilities.getRowStart(doc,
       * bracketPos); boolean isWSC = sup.isCommentOrWhitespace(bolPos, bracketPos); if (isWSC) { //
       * get previous line end } } } } catch (BadLocationException e) { target.getToolkit().beep();
       * } }
       */
    }
  }
  ...
  /**
   * Prefix maker adds the prefix before the identifier under cursor. The prefix is not added if
   * it's already present. The prefix to be added is specified in the constructor of the action
   * together with the prefix group. If there's already any prefix from the prefix group at the
  ...
  /**
   * Executed when the Escape key is pressed. By default it hides the popup menu if visible.
   */
  public static class EscapeAction extends BaseAction {
    private static final long serialVersionUID = -2378870181497378042L;
...
//        Caret caret = target.getCaret();
        ...
        if (!completion.isPaneVisible()) {
          if (completion.isAutoPopupEnabled()) {
            ...
            if (result == ExtSyntaxSupport.COMPLETION_POPUP) {
              ...
              if (result == ExtSyntaxSupport.COMPLETION_CANCEL) {
  ...
  public static class ShiftInsertBreakAction extends BaseAction {
    private static final long serialVersionUID = -6364618206347619574L;
