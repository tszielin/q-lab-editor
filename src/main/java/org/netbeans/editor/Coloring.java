...
/**
 * Immutable class that stores font and foreground and background colors. The coloring can be
 * applied to either the drawing context, component or some other coloring. Applying first checks
 * whether each of the font and the colors is non-null. If it's null the particular thing is left
 * unchanged. For example to change the background color only the Coloring must be created with the
 * font and foreground color being null. It's also possible to use a more advanced way to apply the
 * coloring by using the modes. There are two modes - font-mode and color-mode. The deafult
 * font-mode simply overwrites the whole font. The font-mode constants allow to change the
 * underlying font by only changing some of the font-name, style or size. The
 * <tt>FONT_MODE_APPLY_STYLE</tt> for example will only apply the style of the coloring's font. The
 * "rest" of the coloring's font (name and size in this case) is not used so there can be any valid
 * values ("Monospaced" and 12 for example). The colors can use the alpha value. It's also possible
 * to set the color of the underline line or the color of the strikethrough line. Generally the
 * editor uses two sets of the colorings to colorize the text. They are component-set and
 * printing-set. The component-set is used for the editor component while the printing-set is used
 * solely for colorizing the printed text.
 * 
...
public final class Coloring implements java.io.Serializable {
  ...
  /**
   * Apply only the name from the font. This flag can be combined with other FONT_MODE flags. When
   * using solely this constant the "rest" of the coloring's font (style and size in this case) is
   * unused so there can be any valid values used to create the font (Font.PLAIN and 12 for
   * example).
   */
  public static final int FONT_MODE_APPLY_NAME = 1;

  /**
   * Apply only the style from the font. This flag can be combined with other FONT_MODE flags. When
   * using solely this constant the "rest" of the coloring's font (name and size in this case) is
   * unused so there can be any valid values used ("Monospaced" and 12 for example).
   */
  public static final int FONT_MODE_APPLY_STYLE = 2;

  /**
   * Apply only the size from the font. This flag can be combined with other FONT_MODE flags. When
   * using solely this constant the "rest" of the coloring's font (name and style in this case) is
   * unused so there can be any valid values used to create the font ("Monospaced" and Font.PLAIN
   * for example).
   */
  public static final int FONT_MODE_APPLY_SIZE = 4;

  /**
   * Replace the underlying font by the coloring's font. This value is a binary combination of
   * FONT_MODE_APPLY_NAME, FONT_MODE_APPLY_STYLE, FONT_MODE_APPLY_SIZE.
   */
  public static final int FONT_MODE_DEFAULT = FONT_MODE_APPLY_NAME | FONT_MODE_APPLY_STYLE |
  ...
  /**
   * Cache holding the [original-font, derived-font] pairs and also original [fore-color,
   * derived-fore-color] pairs. This helps to avoid the repetitive computations of the derived font
   * and foreground-color (the backround-color is handled through different cache) and it also
   * avoids the repetitive creations of the derived font and color objects.
   */
  private transient HashMap<Object, Object> fontAndForeColorCache;

  /** Cache holding the [back-color, derived-back-color] pairs */
  private transient HashMap<Color, Color> backColorCache;
  ...
  /** Construct new coloring */
  public Coloring(Font font, int fontMode, Color foreColor, Color backColor, Color underlineColor,
      Color strikeThroughColor, Color waveUnderlineColor) {
  ...
  private void checkCaches() {
    ...
    if (createFontCache || createForeColorCache) {
      fontAndForeColorCache = new HashMap<Object, Object>((createFontCache && createForeColorCache) ? 47 : 23);
    }

    if (backColor != null && hasAlpha(backColor)) {
      backColorCache = new HashMap<Color, Color>(23);
    }
  }
  ...
  /**
   * Apply this coloring to component colors/font. The underline and strikeThrough line colors have
   * no effect here.
   */
   ...
   * 
   * @param c
   *          coloring to which this coloring will be applied. If it's null then this coloring will
  ...
  public Coloring apply(Coloring c) {
    ...
    if (c.fontMode != FONT_MODE_DEFAULT || newFont != c.font // currently only equality
      ...
      return new Coloring(newFont, newFontMode, newForeColor, newBackColor, newUnderlineColor,
          newStrikeThroughColor, newWaveUnderlineColor);
    }
    else {
  ...
  public boolean equals(Object o) {
    if (o instanceof Coloring) {
      ...
      return ((font == null && c.font == null) || (font != null && font.equals(c.font))) &&
          ...
          ((waveUnderlineColor == null && c.waveUnderlineColor == null) || (waveUnderlineColor != null && waveUnderlineColor.equals(c.waveUnderlineColor))) &&
          ((strikeThroughColor == null && c.strikeThroughColor == null) || (strikeThroughColor != null && strikeThroughColor.equals(c.strikeThroughColor)));
    }
    return false;
  }
  ...
  /**
   * Derive a new coloring by changing the font and leaving the rest of the coloring (including the
   * font-mode) unchanged.
   */
  ...
  /**
   * Derive a new coloring by changing the font and font-mode and leaving the rest of the coloring
   * unchanged.
   */
  ...
  /**
   * Derive a new coloring by changing the foreground-color and its color-mode and leaving the rest
   * of the coloring unchanged.
   */
  ...
  /**
   * Derive a new coloring by changing the background-color and its color-mode and leaving the rest
   * of the coloring unchanged.
   */
