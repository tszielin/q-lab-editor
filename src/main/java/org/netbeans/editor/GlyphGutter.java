...
import tszielin.qlab.component.AnnotationDescription;
import tszielin.qlab.config.data.ConnectionStatus;

/**
 * GlyphGutter is component for displaying line numbers and annotation glyph icons. Component also
 * allow to "cycle" through the annotations. It means that if there is more than one annotation on
 * the line, only one of them might be visible. And clicking the special cycling button in the
 * gutter the user can cycle through the annotations.
 * 
...
public class GlyphGutter extends JComponent implements Annotations.AnnotationsListener, Accessible {
  ...
  private static final long serialVersionUID = 6857363266605745194L;
  ...
  /**
   * Flag whther the gutter was initialized or not. The painting is disabled till the gutter is not
   * initialized
   */
  ...
  /*
   * These two variables are used for caching of count of line annos on the line over which is the
   * mouse caret. Just for sake of optimalization.
   */
  ...
  public AccessibleContext getAccessibleContext() {
    if (accessibleContext == null) {
      accessibleContext = new AccessibleJComponent() {
        private static final long serialVersionUID = -2721220560654812122L;
  ...
  public void setConnected(ConnectionStatus status) {
  ...
  protected void init() {
    ...
    // imageURL = new URL("/org/netbeans/editor/resources/glyphbutton.gif"); // NOI18N
    ...
    if (getClass().getResource("/org/netbeans/editor/resources/glyphbutton.gif") != null) {
      gutterButton = Toolkit.getDefaultToolkit().getImage(
          getClass().getResource("/org/netbeans/editor/resources/glyphbutton.gif"));
    }
    ...
    AnnotationTypes.getTypes().addPropertyChangeListener(
        annoTypesListener = new PropertyChangeListener() {
   ...
   */
  public void update() {
    Coloring lineColoring = (Coloring)editorUI.getColoringMap().get(
    ...
    if (lineColoring.getBackColor() != null)
      backgroundColor = status == null || status == ConnectionStatus.NOT_CONNECTED ? Color.red :          
          status == ConnectionStatus.LOST_CONNECTION ? Color.gray : lineColoring.getBackColor();
    else
      backgroundColor = status == null || status == ConnectionStatus.NOT_CONNECTED ? Color.red :
          status == ConnectionStatus.LOST_CONNECTION ? Color.gray : defaultColoring.getBackColor();
  ...
  public void paintComponent(Graphics g) {
    ...
    while ((y + (lineHeight / 2)) <= (drawHere.y + drawHere.height)) {
      ...
      if (showLineNumbers &&
          ((!drawOverLineNumbers) || (drawOverLineNumbers && line != lineWithAnno))) {
        if (line < lineCount) {
          int lineNumberWidth = fm.stringWidth("" + (line + 1));
          g.drawString("" + (line + 1), numberWidth - lineNumberWidth - rightMargin, y +
              editorUI.getLineAscent());
        }
      }
        ...
        if (anno != null) {
          ...
          if (!(count == 1 && anno.isDefaultGlyph())) {
            if (anno.getGlyph() != null && prepareImage(anno.getGlyph(), imgObserver))
              g.drawImage(anno.getGlyph(), xPos, y +
                  (lineHeight - anno.getGlyph().getHeight(null)) / 2 + 1, null);
          }
        }

        // draw cycling button if there is more than one annotations on the line
        if (count >= 1)
          if (anno.getGlyph() != null && prepareImage(gutterButton, imgObserver) &&
              prepareImage(anno.getGlyph(), imgObserver))
            g.drawImage(gutterButton, xPos + glyphWidth, y +
                (lineHeight - anno.getGlyph().getHeight(null)) / 2, null);
...
//    private boolean selectForward;
    ...
    public void mouseClicked(MouseEvent e) {
      ...
      if (debuger && (e.getModifiers() & InputEvent.BUTTON1_MASK) == InputEvent.BUTTON1_MASK &&
          e.getClickCount() == 2) {
        ...
        AnnotationDesc annotation = annos.getAnnotation(line, AnnotationTypes.PROP_COMBINE_GLYPHS);
        if (annotation != null) {
          annos.removeAnnotation(annotation);
        }
        else {
          annos.addAnnotation(new AnnotationDescription(line));
        }
        // } else {
...
//              if (offSet < 0) {
//                offSet = Utilities.getRowEnd(doc, Utilities.getRowStartFromLineOffset(doc,
//                    dragStartLine));
//              }
