...
public class CloneableEditor extends CloneableTopComponent {
  ...
  /**
   * Position of cursor. Used to keep the value between deserialization and initialization time.
   */
  private int cursorPosition = -1;

  // #20647. More important custom component.
  /**
   * Custom editor component, which is used if specified by document which implements
   * <code>NbDocument.CustomEditor</code> interface.
   * 
   ...
   * 
   * @param support
   *          support that holds the document and operations above it
   */
  ...
  /**
   * Gives access to {@link CloneableEditorSupport} object under this <code>CloneableEditor</code>
   * component.
   * 
   * @return the {@link CloneableEditorSupport} object that holds the document or <code>null</code>,
   *         what means this component is not in valid state yet and can be discarded
   */
  ...
  /**
   * Get context help for this editor pane. If the registered editor kit provides a help ID in bean
   * info according to the protocol described for {@link InstanceSupport#findHelp}, then that it
   * used, else general help on the editor is provided.
   * 
   * @return context help
   */
  ...
  /**
   * Indicates whtehter this componetn can be closed in the workspace. Overrides superclass method.
   * Adds scheduling of "emptying" <code>pane</code> field and removing all sub components.
   */
  public boolean canClose(Workspace workspace, boolean last) {
    boolean result = super.canClose(workspace, last);
    int closeOp = getCloseOperation();
    if (result && ((closeOp == CLOSE_EACH) || (closeOp == CLOSE_LAST && last))) {
  ...
  /**
   * Overrides superclass method. In case it is called first time, initializes this
   * <code>CloneableEditor</code>.
   */
  ...
  private void initialize() {
    ...
    pane.getAccessibleContext().setAccessibleName(
        NbBundle.getMessage(CloneableEditor.class, "ACS_CloneableEditor_QuietEditorPane",
            this.getName()));
    pane.getAccessibleContext().setAccessibleDescription(
        NbBundle.getMessage(CloneableEditor.class, "ACSD_CloneableEditor_QuietEditorPane",
  ...
  /**
   * Descendants overriding this method must either call this implementation or fire the
   * {@link org.openide.cookies.EditorCookie.Observable#PROP_OPENED_PANES} property change on their
   * own.
   */
  ...
  /**
   * Overrides superclass version. Opens top component only if it is in valid state. (Editor top
   * component may become invalid after deserialization).<br>
   * Also tries to open all other top components which are docked in editor mode on given workspace,
   * but not visible.<br>
   */
  public void open(Workspace workspace) {
    if (discard()) {
      ErrorManager.getDefault().log(ErrorManager.WARNING, "Can not open " + this + " component," // NOI18N
      ...
      Workspace realWorkspace = (workspace == null)
          ? WindowManager.getDefault().getCurrentWorkspace() : workspace;
  ...
  public SystemAction[] getSystemActions() {
    ...
    return SystemAction.linkActions(sa, new SystemAction[]{null,
  ...
  /**
   * Overrides superclass method. Remembers last selected component of support belonging to this
   * component.
   * 
   ...
   * {@link org.openide.windows.TopComponent TopCompoenent} according to the support retrieved from
   * {@link #cloneableEditorSupport} method. The name and tooltip are in case of support presence
    ...
    if (p != null) {
      ...
      if (caret != null) {
        ...
        if (p instanceof QuietEditorPane) {
          ...
          if (doc != null) {
            ErrorManager.getDefault().notify(ErrorManager.INFORMATIONAL,
                new IllegalStateException("Caret is null in editor pane=" + p // NOI18N
  ...
  /**
   * Replaces serializing object. Overrides superclass method. Adds checking for object validity. In
   * case this object is invalid throws {@link java.io.NotSerializableException
   * NotSerializableException}.
   * 
   * @throws ObjectStreamException
   *           When problem during serialization occures.
   * @throws NotSerializableException
   *           When this <code>CloneableEditor</code> is invalid and doesn't want to be serialized.
   */
  ...
  /**
   * Resolves deserialized object. Overrides superclass method. Adds checking for object validity.
   * In case this object is invalid throws {@link java.io.InvalidObjectException
   * InvalidObjectException}.
   * 
   * @throws ObjecStreamException
   *           When problem during serialization occures.
   * @throws InvalidObjectException
   *           When deserialized <code>CloneableEditor</code> is invalid and shouldn't be used.
   */
  ...
  /**
   * This component should be discarded if the associated environment is not valid.
   */
  ...
  /**
   * Dock this top component to editor mode if it is not docked in some mode at this time
   */
  ...
  private Mode editorMode(Workspace workspace) {
    Mode ourMode = workspace.findMode(this);
    if (ourMode == null) {
      ourMode = workspace.createMode(CloneableEditorSupport.EDITOR_MODE, getName(),
  ...
  /**
   * Utility method, opens this top component on all workspaces where editor mode is visible and
   * which differs from given workspace.
   */
  private void openOnOtherWorkspaces(Workspace workspace) {
    ...
    for (int i = 0; i < workspaces.length; i++) {
      ...
      if (!isOpened(workspaces[i]) && curEditorMode != null &&
          (tcMode == null || tcMode.equals(curEditorMode))) {
  ...
  /**
   * Utility method, opens top components which are opened in editor mode on some other workspace.
   * This method should be called only if first top component is being opened in editor mode on
   * given workspace
   */
  private void openOtherEditors(Workspace workspace) {
    // choose candidates for opening
    Set<TopComponent> topComps = new HashSet<TopComponent>(15);
    ...
    for (Iterator<TopComponent> iter = topComps.iterator(); iter.hasNext();) {
      pureOpen(iter.next(), workspace);
    }
  }

  /**
   * Utility method, calls super version of open if given top component is of Editor type, or calls
   * regular open otherwise. The goal is to prevent from cycle open call between Editor top
   * components
   */
