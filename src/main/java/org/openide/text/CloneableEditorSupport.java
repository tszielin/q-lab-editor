...
/**
 * Support for associating an editor and a Swing {@link Document}. Can be assigned as a cookie to
 * any editable data object. This class is abstract, so any subclass has to provide implementation
 * for abstract method (usually for generating of messages) and also provide environment {@link Env}
 * to give this support access to input/output streams, mime type and other features of edited
 * object.
 * <P>
 * This class implements methods of the interfaces {@link org.openide.cookies.EditorCookie},
 * {@link org.openide.cookies.OpenCookie}, {@link org.openide.cookies.EditCookie},
 * {@link org.openide.cookies.ViewCookie}, {@link org.openide.cookies.LineCookie},
 * {@link org.openide.cookies.CloseCookie}, and {@link org.openide.cookies.PrintCookie} but does not
 * implement those interfaces. It is up to the subclass to decide which interfaces really implement
 * and which not.
 * 
...
public abstract class CloneableEditorSupport extends CloneableOpenSupport {
  ...
  /**
   * Used for allowing to pass getDocument method when called from loadDocument from loadTask.
   */
  private static final ThreadLocal<Object> LOCAL_LOAD_TASK = new ThreadLocal<Object>();

  /** Task for loading the document. */
  private Task loadTask;

  /**
   * Task for preparing the document. Consists for loading a document (runs loadTask), firing
   * </code>stateChange</code> and initializing it by attaching listeners listening to document
   * changes, such as SavingManager and LineSet.
   */
  ...
  /**
   * Helper variable to prevent multiple cocurrent printing of this instance.
   */
  ...
  /**
   * The string which will be appended to the name of top component when top component becomes
   * modified
   */
  ...
  /**
   * Whether the reload dialog is currently opened. Prevents poping of multiple reload dialogs if
   * there is more external saves.
   */
   ...
   * 
   * @param env
   *          environment that is source of all actions around the data object
   */
  ...
  /**
   * Provides access to position manager for the document. It maintains a set of positions even the
   * document is in memory or is on the disk.
   * 
  ...
  /**
   * Add a PropertyChangeListener to the listener list. See
  ...
  /**
   * Remove a PropertyChangeListener from the listener list. See
   * {@link org.openide.cookies.EditorCookie.Observable}.
   * 
   * @param l
  ...
  /**
   * @param clearDocument
   *          indicates whether the document is needed to clear before (used for reloading)
   */
  ...
  /**
   * Get the document associated with this cookie. It is an instance of Swing's
   * {@link StyledDocument} but it should also understand the NetBeans {@link NbDocument#GUARDED} to
   ...
   * <code>prepareTask</code>) has been completed, in such a case the document must not be modified.
   * 
   ...
   * 
   * @return <code>true</code> if the document is in memory and is modified; otherwise
   *         <code>false</code>
   */
  ...
  /*
   * Whether the file was externally modified or not. This flag is used only in saveDocument to
   * prevent overriding of externally modified file. See issue #32777.
   */
  ...
  /*
   * List of all JEditorPane's opened by this editor support. The first item in the array should
   * represent the component that is currently selected or has been selected lastly. If you override
   * this method and return also your own editor panes which are not descendants of {@link
   * CloneableEditor} then it is your responsibility to fire {@link
   * org.openide.cookies.EditorCookie.Observable#PROP_OPENED_PANES} property change whenever the
   * list of opened panes changes.
   * @return array of panes or null if no pane is opened. In no case empty array is returned.
   */
  ...
  public void print() {
    ...
    try {
      ...
      String msg = NbBundle.getBundle(CloneableEditorSupport.class).getString("CTL_Bad_File"); // NOI18N
      notifyInAWT(msg);
    }
    catch (IOException e) {
      ErrorManager.getDefault().notify(e);
      ...
      String msg = NbBundle.getBundle(CloneableEditorSupport.class).getString("CTL_Printer_Abort"); // NOI18N
      notifyInAWT(msg);
    }
  ...
  /**
   * Prepares document, creates and initializes new <code>CloneableEditor</code> component.
   * Typically do not override this method. For creating your own <code>CloneableEditor</code> type
   * component override {@link #createCloneableEditor} method.
   * 
  ...
  protected boolean canClose() {
    if (env().isModified()) {
      ...
      NotifyDescriptor nd = new NotifyDescriptor(msg, bundle.getString("LBL_SaveFile_Title"),
          NotifyDescriptor.YES_NO_CANCEL_OPTION, NotifyDescriptor.QUESTION_MESSAGE, new Object[]{
              saveOption, discardOption, NotifyDescriptor.CANCEL_OPTION}, saveOption);
    ...
    /*
     * old code was: SaveCookie savec = (SaveCookie)
     * entry.getDataObject().getCookie(SaveCookie.class); if (savec != null) { MessageFormat format
     * = new MessageFormat(NbBundle.getBundle(EditorSupport.class).getString("MSG_SaveFile")); //
     * NOI18N String msg = format.format(new Object[] { entry.getDataObject().getName()});
     * NotifyDescriptor nd = new NotifyDescriptor.Confirmation(msg,
     * NotifyDescriptor.YES_NO_CANCEL_OPTION); Object ret = DialogDisplayer.getDefault().notify(nd);
     * if (NotifyDescriptor.CANCEL_OPTION.equals(ret) || NotifyDescriptor.CLOSED_OPTION.equals(ret)
     * ) { return false; } if (NotifyDescriptor.YES_OPTION.equals(ret)) { try { savec.save(); }
     * catch (IOException e) { ErrorManager.getDefault().notify(e); return false; } } }
     */
  }
   ...
   * @param l
   *          new listener
   * @deprecated Deprecated since 3.40. Use {@link #addPropertyChangeListener} instead. See also
   *             {@link org.openide.cookies.EditorCookie.Observable}.
   */
   ...
   * @param l
   *          listener to remove
   * @deprecated Deprecated since 3.40. Use {@link #removePropertyChangeListener} instead. See also
   *             {@link org.openide.cookies.EditorCookie.Observable}.
   */
   ...
   * 
   * @param offset
   *          the offset to create position at
   * @param bias
  ...
  /**
   * Allows subclasses to create their own version of <code>CloneableEditor</code> component.
   * 
   * @return the {@link CloneableEditor} for this support
   */
  ...
  /**
   * Initialize the editor. This method is called after the editor component is deserialized and
   * also when the component is created. It allows the subclasses to annotate the component with
   * icon, selected nodes, etc.
   * 
   * @param editor
  ...
  /**
   * Create an undo/redo manager. This manager is then attached to the document, and listens to all
   * changes made in it.
   * <P>
   ...
   * <p>
   * The default implementation just calls {@link EditorKit#write(OutputStream, Document, int, int)
   * EditorKit.write(...)}. Subclasses could override this to provide support for persistent guard
   * blocks, for example.
   * 
   * @param doc
   *          the document to write from
   * @param kit
   ...
   * <p>
   * The default implementation just calls {@link EditorKit#read(InputStream, Document, int)
   * EditorKit.read(...)}. Subclasses could override this to provide support for persistent guard
   * blocks, for example.
   * 
   * @param doc
   *          the document to read into
   * @param stream
   ...
   * 
   * @return task that reloads the document. It can be also obtained by calling
   *         <tt>prepareDocument()</tt>.
   */
  protected Task reloadDocument() {
    synchronized (getLock()) {
      if (doc != null) {
        // UndoManager must be detached from document here because it will be attached in
        // loadDocument()
          ...
          public void taskFinished(Task task) {
            ...
            SwingUtilities.invokeLater(new Runnable() {
              public void run() {
                ...
                // Insert before-save undo event to enable unmodifying undo
                getUndoRedo().undoableEditHappened(
                    new UndoableEditEvent(CloneableEditorSupport.this, new BeforeSaveEdit(
                        lastSaveTime)));
  ...
  /**
   * Method that can be overriden by children to create empty styled document or attach additional
   * document properties to it.
   * 
   * @param kit
   *          the kit to use
   ...
   * reload of document.
   * <P>
   * This implementation simply marks the associated environement unmodified and updates titles of
   * all components.
   */
  ...
  /**
   * Called when the document is being modified. The responsibility of this method is to inform the
   * environment that its document is modified. Current implementation Just calls env.setModified
   * (true) to notify it about modification.
   * 
  ...
  /**
   * Resets listening on <code>UndoRedo</code>, and in case next undo edit comes, schedules
   * processesing of it. Used to revert modification e.g. of document of [read-only] env.
   */
    ...
    return new Runnable() {
      public void run() {
        ...
        try {
          ...
          ErrorManager.getDefault().notify(ErrorManager.INFORMATIONAL, cne);
        }
        finally {
  ...
  /**
   * Method that is called when all components of the support are closed. The default implementation
   * closes the document.
   */
  ...
  /*
   * JST: Commented out Set actions for toolbar.
   * @param actions list of actions public void setActions (SystemAction[] actions) { this.actions =
   * actions; } /** Utility method which enables or disables listening to modifications on asociated
   * document. <P> Could be useful if we have to modify document, but do not want the Save and Save
   * All actions to be enabled/disabled automatically. Initially modifications are listened to.
   * @param listenToModifs whether to listen to modifications public void setModificationListening
   * (final boolean listenToModifs) { if (this.listenToModifs == listenToModifs) return;
   * this.listenToModifs = listenToModifs; if (doc == null) return; if (listenToModifs)
   * doc.addi(getModifL()); else doc.removeDocumentListener(getModifL()); }
   */
   ...
   *          kit to use
   * @param d
   *          original document to load data into
   */
  private void loadDocument(EditorKit kit, StyledDocument doc) throws IOException {
    ...
    try {
      ...
      if (aProblem != null) {
        ...
        err.annotate(aProblem, NbBundle.getMessage(CloneableEditorSupport.class,
  ...
  private void closeDocument() {
    for (;;) {
      ...
      /*
       * Wait for loading task to be finished so that the document etc. stays valid during the load
       * operation.
       */
      prep.waitFinished();
    }
   ...
   * 
   * @param doReload
   *          false if we should first ask the user
   */
  private void checkReload(boolean doReload) {
    ...
    if (!doReload && !reloadDialogOpened) {
      String msg = NbBundle.getMessage(CloneableEditorSupport.class, "FMT_External_change", // NOI18N
   ...
   * 
   * @param d
   *          document to use as underlaying one
  ...
  private final void fireStateChangeEvent(StyledDocument document, boolean closing) {
    if (listeners != null) {
      ...
      HashSet<ChangeListener> s = null;
      synchronized (this) {
        s = ((HashSet<ChangeListener>)listeners.clone());
      }

      Iterator<ChangeListener> it = s.iterator();
      while (it.hasNext()) {
  ...
  /**
   * Accessor to the <code>Listener</code> instance, lazy created on demand. The instance serves as
   * a listener on document, environment and also provides document initialization task for this
   * support.
   * 
   * @see Listener
   */
  ...
  /**
   * The listener that this support uses to communicate with document, environment and also
   * temporarilly on undoredo.
   */
  private final class Listener extends Object implements ChangeListener, DocumentListener,
    ...
    /**
     * Returns exception from loadDocument, caller thread can check it after load task finishes.
     * Returns null if no exception happened. It resets loadExc to null.
     */
    ...
    /**
     * Schedules reverting(undoing) of prohibited change. Implements <code>ChangeListener</code>.
     * 
     * @see #revertUpcomingUndo
     */
    ...
    public void propertyChange(PropertyChangeEvent ev) {
      ...
      if (Env.PROP_MODIFIED.equals(ev.getPropertyName())) {
        CloneableEditorSupport.this.firePropertyChange(EditorCookie.Observable.PROP_MODIFIED, null,
            null);
      }
    }
    ...
    public void run() {
      synchronized (getLock()) {
        /*
         * Remove existing listener before running the loading task This should prevent firing of
         * insertUpdate() during load (or reload) which can prevent dedloks that sometimes occured
         * during file reload.
         */
        doc.removeDocumentListener(getListener());
        try {
  ...
  /**
   * Undoable edit that is put before the savepoint. Its replaceEdit() method will consume and wrap
   * the edit that precedes the save. If the edit is added to the begining of the queue then the
   * isSignificant() implementation guarantees that the edit will not be removed from the queue.
  ...
  private class BeforeSaveEdit extends FilterUndoableEdit {
    ...
    public boolean addEdit(UndoableEdit anEdit) {
      if (!(anEdit instanceof BeforeModificationEdit)) {
        /*
         * UndoRedo.addEdit() must not be done lazily because the edit must be "inserted" before the
         * current one.
         */
  ...
  /**
   * Edit that is created by wrapping the given edit. When undone it marks the document as not
   * modified.
   */
