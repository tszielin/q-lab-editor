...
/**
 * Entity resolver resolving all entities registered by modules. Use {@link #getDefault} to get the
 * master instance in system. Any parser working with unknown XML documents should use it to avoid
 * unnecessary Internet connections.
 * <p>
 * You can register your own instances via lookup to add to the resolver pool, for example using an
 * XML file in the format defined by {@link #PUBLIC_ID}, but for reasons of performance and
 * predictability during startup it is best to provide the entity (e.g. some DTD you define) as the
 * contents of a file in the system filesystem, in the <samp>/xml/entities/</samp> folder, where the
 * file path beneath this folder is based on the public ID as follows:
 * <ol>
 ...
 * <samp>-//NetBeans//Entity&nbsp;Mapping&nbsp;Registration&nbsp;1.0//EN</samp> would be looked for
 * in the file <samp>/xml/entities/NetBeans/Entity_Mapping_Registration_1_0</samp>. Naturally this
 * only works if you are defining a fixed number of entities.
 * <p>
 * It is recommended that the entity file in <samp>/xml/entities/</samp> also be given a file
...
public abstract class EntityCatalog implements EntityResolver {
  ...
  /**
   * Get a master entity catalog which can delegate to any others that have been registered via
   * lookup.
   */
