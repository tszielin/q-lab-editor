...
/**
 * This backend class for XMLUtil contains implementation of DOM implementation "independent" write
 * method.
 * 
 * @author Petr Kuzel
...
class XMLUtilImpl extends Object {
   ...
   * 
   * @param out
   *          must be Writer or OutputStream
   * @param XML
   *          encoding name (ignored parametr)
   */
  static void write(Document doc, Object out, String encoding) throws IOException {
    ...
    try {
      if ("com.sun.xml.tree.XmlDocument".equals(cls.getName()) // NOI18N
          ||
          "org.apache.crimson.tree.XmlDocument".equals(cls.getName())) { // NOI18N

        // these DOM implementations are self writing
        Method write;

        if (out instanceof Writer) {
          write = cls.getDeclaredMethod("write", new Class[]{Writer.class});// NOI18N
        }
        else {
          write = cls.getDeclaredMethod("write", new Class[]{OutputStream.class});// NOI18N
        }
        ...
        Class<?> serka = Class.forName("org.apache.xml.serialize.XMLSerializer", true, cl); // NOI18N

        Class<?> forka = Class.forName("org.apache.xml.serialize.OutputFormat", true, cl); // NOI18N
